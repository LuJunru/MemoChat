...
data = json.load(open('raw_data/topiocqa_train.json', 'r'))
...
for ri, row in enumerate(data):
    if (ri + 1 == len(data)) or (row["Conversation_no"] != data[ri + 1]["Conversation_no"]):
        new_row = {"id": row["Conversation_no"], "conversations": [{"from": "human", "value": ""}, {"from": "bot", "value": ""}], "type": ""}
        case_num = choice([1, 2])
        ...
        if case_num == 1:
            ...
            new_row["conversations"][0]["value"] = system_insturction.replace("LINE", str(li + 1)) + "\n\n```\nTask Conversation:\n" + "\n".join(new_conversation) + "\n```" + task_instruction.replace("LINE", str(li + 1))
            ...
            new_row["conversations"][1]["value"] = json.dumps(row_answer)
...
json.dump(new_data, open('topiocqa_train_question.json', 'w'), indent=2)
