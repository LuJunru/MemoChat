...
data = open('raw_data/dialogsum.dev.jsonl', 'r').readlines()
shuffle(data)
...
num_count_2 = 0
num_count_3 = 0
w = open('dialogsum_test_question.jsonl', 'w')
while count < len(data):
    num_shift = choice([1] * 1 + [2] * 5 + [3] * 2 + [4] * 1 + [5] * 1)
    count += num_shift
    lines = data[last_count:count]
    
    new_row = {"question_id": "", "text": "", "type": "", "answer": ""}
    for line in lines:
        record = json.loads(line.strip())
        fname = record["fname"]
        if new_row["question_id"] == "":
            new_row["question_id"] = fname
        else:
            new_row["question_id"] += "_" + fname.split("_")[1]
    ...
    if case_num == 1 and num_shift <= 3:
        ...
        new_row["text"] = system_insturction.replace("LINE", str(len(conversation))) + "\n\n```\nTask Conversation:\n" + "\n".join(conversation) + "\n```" + task_instruction.replace("LINE", str(len(conversation)))
        new_row["answer"] = json.dumps(answers)
    ...
    if case_num == 3:
        ...
        task_case = "```\nRelated Evidences:\n" + "\n".join(["({}) {}".format(r_tsd_i + 1, {
                    ...
                    "\n```\n\nUser Input:\n" + data_set["User Input"]
        new_row["text"] = system_insturction + task_case + task_instruction
        new_row["answer"] = data_set["answer"]
    ...
    if len(new_row["text"].split(" ") + new_row["answer"].split(" ")) > 1400:
        count -= num_shift
        continue
    else:
        if case_num == 1:
